...
# --- PDF Generation ---
# Individual reformatted pages are returned as BytesIO buffers — nothing is
# written to disk until the final merged packet.
def create_payment_pdf(payment_data, invoices):
    ...
    # Line 1: "Received"  — title case, bold, 13pt
    # Line 2: date        — all caps (e.g. "MAY 28 2026"), bold, 11pt
    # Line 3: dept        — "Acct Receivable Dept.", regular weight, 8pt
    ...
    doc.build(elements)
    buffer.seek(0)
    return buffer  # caller owns the buffer; nothing written to disk here
...
class StateFarmFormatterApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        self.round_rectangle(logo_canvas, 0, 0, 36, 36, radius=8, fill="#CC0000")
        ...
        # Log Console
        # Tag colors: "success" → green, "error" → red, "info" → slate, "warning" → yellow
        ...
        self.log_console.tag_config("warning", foreground="#FCD34D")
    ...
    def process_pdfs(self, folder):
        try:
            ...
            # Accumulate in-memory buffers only — no intermediate files touch the filesystem
            processed_buffers = []
            successful_files = 0
            skipped_files = 0
            ...
            for i, pdf_path in enumerate(pdf_files):
                ...
                if result['status'] == 'failed':
                    # Skipped files are never added to processed_buffers and are excluded from merge
                    skipped_files += 1
            ...
            # Only write once: the final merged packet
    ...
    def parse_single_pdf(self, pdf_path):
        ...
        # Variant B: firm-name header (adds ~4 extra lines before first Billing block)
        # Variant A: account-number header
        # Invoice block parsing below uses a pure state machine and is agnostic to
        # which variant is present — it never uses fixed line-index offsets.
        ...
        # Pure state machine: scans every non-blank line sequentially, matching
        # field labels regardless of where they appear in the document.
        # Billing refs: captured from lines beginning "Reference <value>" that
        # immediately follow a "Billing" label line.
        ...
        while i < len(lines):
            ...
            if line == 'Billing' and i + 1 < len(lines) and lines[i + 1].startswith('Reference '):
                # Capture everything after "Reference " on the same line
        ...
        # Compile invoices from parallel field lists
        ...
        # Check 1 (CRITICAL): billing ref must not look like a date — if the
        # pattern ^\d{2}-\d{2}-\d{4}$ matches, the parser grabbed an invoice
        # date instead of a ref number.
        ...
        # Check 2 (WARNING): all eight field lists must be the same length.
        ...
        # Check 3 (CRITICAL): payment total must be present.
        if not m_total:
            return {'status': 'failed', 'reason': 'no payment total found'}

        # Check 4 (WARNING): parsed total must match the dollar amount in the
        # filename within $0.02 tolerance.
        ...
        # Check 5 (CRITICAL): at least one invoice block must have been found.
